...
| `-ONLY_CF_IP_ALLOW y` | Fetch Cloudflare CIDRs and restrict WS path to CDN IPs only |
| `-tgbot TOKEN` | Telegram bot token — enables 3x-ui login/traffic alerts |
| `-tgchat ID` | Telegram chat/group ID (use with `-tgbot`) |
| `-update_config y` | Re-seed DB + reload nginx without reinstalling 3x-ui |
| `-dry_run y` | Print destructive actions without executing (safe on live servers) |
...
```
install.sh                         # the installer (v1.3.0)
